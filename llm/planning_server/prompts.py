"""
LLM 프롬프트 템플릿
레시피를 음성 안내용 대화 스크립트로 변환 (GPT-5 Nano 최적화)
"""
...
3. **응답 형식**: 반드시 유효한 JSON만 출력하세요. 설명, 주석, 마크다운 절대 금지.
4. **필드 누락 금지**: 모든 필수 필드를 반드시 포함하세요.
5. **제목 변경 금지**: 레시피 제목을 임의로 변경하지 마세요.
...
- 시간이 명시되면 estimated_time_sec에 초 단위로 정확히 입력
- "준비하세요", "썰어주세요" 등 즉시 행동은 timer_required: false

6. 인사말 가이드
- opening_remark: 레시피 제목을 그대로 포함한 긍정적 시작 인사
- closing_remark: 완성을 축하하고 다음 행동 제안

출력 형식: **오직 JSON만** 응답하세요. 다른 설명은 불필요합니다.
"""


def create_planning_prompt(title: str, ingredients: list, steps: list) -> str:
    """
    레시피 데이터를 받아 LLM 프롬프트 생성 (GPT-5 Nano용 Few-Shot 포함)
...
조리 단계:
1. 계란을 그릇에 풀어주세요.
2. 팬에 기름을 두르고 중불에서 익혀주세요.

출력 (JSON만):
{{
  "opening_remark": "안녕하세요! 지금부터 계란 스크램블 만들기를 시작하겠습니다.",
  "planned_steps": [
    {{
      "order": 1,
      "script": "첫 번째 단계입니다. 계란 두 개를 그릇에 풀어주세요.",
      "retry_script": "다시 설명드릴게요. 계란 두 개를 그릇에 넣고, 젓가락이나 포크로 노른자와 흰자가 골고루 섞이도록 저어주세요.",
      "fallback_script": "계란을 잘 섞어주시면 됩니다. 천천히 젓가락으로 저어주세요.",
    ...
    {{
      "order": 2,
      "script": "다음 단계입니다. 팬에 기름을 두르고 중불에서 계란을 부어 익혀주세요.",
      "retry_script": "다시 한 번 설명드릴게요. 팬을 중불로 달군 뒤, 기름을 약간 두르세요. 그 다음 계란물을 부어서 젓가락으로 저으며 익혀주시면 됩니다.",
      "fallback_script": "팬에 기름을 두르고 계란을 부어서 저으며 익혀주세요.",
...
2. planned_steps: 위에 제공된 조리 단계만 사용하여 음성 안내용 대화 스크립트로 변환
   ...
   - timer_required: 타이머 필요 여부 (true 또는 false, 불린 타입)
   - timer_message: 타이머 시작 시 안내 멘트 (timer_required가 false면 빈 문자열 "")
...
- 숫자: '180g'은 '백팔십 그램', '1/2'은 '반', '2L'은 '이 리터'
- 예시: "물 2L" → "물 이 리터", "소금 5g" → "소금 오 그램"

타이머 규칙:
- "볶으세요", "끓이세요", "익히세요" 등 대기 시간이 있으면 timer_required: true
- 시간이 명시되면 estimated_time_sec에 초 단위로 입력 (예: 2분 = 120, 10분 = 600)
- "준비하세요", "썰어주세요" 등 즉시 행동은 timer_required: false

중요: 
- **오직 유효한 JSON만 출력**하세요. 
- 설명, 주석, 마크다운 코드 블록(```)을 절대 사용하지 마세요.
- 영문자나 특수문자는 반드시 한글로 변환하세요.
