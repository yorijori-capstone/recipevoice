...
export interface CookingSession {
  ...
  planResult?: any; // Planning Service 원본 결과 (title, opening_remark, closing_remark 포함)
...
Always respond in Korean with a friendly, supportive tone.`;
  ...
  async startCookingSession(recipeId: string, recipe: any): Promise<CookingSession> {
    try {
      ...
      session.planResult = planResult; // 원본 planning 결과 저장
      session.status = 'active';

      console.log(`[CookingAgent] Session ${sessionId} started with ${planResult.planned_steps.length} steps`);
      console.log('[CookingAgent] planResult stored:', {
        title: planResult.title,
        opening_remark: planResult.opening_remark?.substring(0, 50),
        steps_count: planResult.planned_steps?.length,
        closing_remark: planResult.closing_remark?.substring(0, 50)
      });
  ...
  nextStep(sessionId: string): any | null {
    ...
    console.log(`[CookingAgent] Moved to step ${session.currentStepIndex + 1}/${session.plannedSteps.length}`);
    return nextStep;
  }
   ...
   */
  previousStep(sessionId: string): any | null {
    const session = this.sessions.get(sessionId);
    if (!session) return null;

    if (session.currentStepIndex <= 0) {
      console.log('[CookingAgent] Already at first step');
      return session.plannedSteps[0];
    }

    session.currentStepIndex--;

    const previousStep = session.plannedSteps[session.currentStepIndex];
    this.emit('step_changed', {
      sessionId,
      stepIndex: session.currentStepIndex,
      step: previousStep
