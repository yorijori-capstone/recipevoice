      ...
      if (lastTimer.action === 'start' && lastTimer.end_time) {
        try {
          ...
          if (isNaN(endTime.getTime())) {
            console.warn(`[MCP Navigation] Invalid end_time format: ${lastTimer.end_time}`);
          } else if (now < endTime) {
            // 타이머 중지 이벤트 기록
            await client.query(
          ...
          console.error(`[MCP Navigation] Error processing timer stop:`, error);
          // 에러 발생해도 계속 진행
        }
      }
    }
