...
export class RecipeCleaner {
  ...
  async getCleanedRecipe(recipeId: string): Promise<CleanedRecipe | null> {
    try {
      ...
      // 🆕 Get ingredients from planning_result or raw DB for legacy recipes
      let ingredients: Ingredient[] = [];

      if (planning_result.ingredients) {
        // AI 생성 레시피: planning_result에서 가져오기
        ...
        // 기존 레시피: ingredients 테이블에서 가져오기
        const ingredientsResult = await pool.query(
          'SELECT name, quantity FROM ingredients WHERE recipe_id = $1 ORDER BY id',
          [recipeId]
        );
...
Output the JSON following the exact schema structure and formatting rules provided in the system prompt.`;
  ...
  async saveCleanedRecipe(
    ...
    try {
      ...
      // Foreign key 제약 조건을 위해 recipes 테이블에 레코드가 없으면 자동 생성
      await client.query(
        ...
        [
          recipeId,
          planningResult.meta.title,
          planningResult.meta.servings.toString(),
          `${planningResult.meta.time_estimate}분`,
          planningResult.meta.difficulty.toLowerCase(),
          'AI Generated',
          'Generated by Recipe Creator',
      ...
      let description = planningResult.meta.description || '맛있는 요리';

      // description이 ~입니다.로 끝나도록 보장
      const trimmed = description.trim();
      if (!trimmed.endsWith('입니다.')) {
        if (trimmed.endsWith('입니다')) {
          description = `${trimmed}.`;
        } else {
          description = `${trimmed}입니다.`;
        }
      } else {
        description = trimmed;
      }
